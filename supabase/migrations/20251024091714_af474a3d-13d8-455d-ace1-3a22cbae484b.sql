...
-- Create separate unique constraints for different stat types
...
-- Drop and recreate increment_staff_stats function
DROP FUNCTION IF EXISTS public.increment_staff_stats(uuid, uuid);

CREATE FUNCTION public.increment_staff_stats(
  staff_id UUID,
  biz_id UUID
)
RETURNS void
...
-- Drop and recreate increment_business_stats function
DROP FUNCTION IF EXISTS public.increment_business_stats(uuid, uuid);

CREATE FUNCTION public.increment_business_stats(
  biz_id UUID,
  cust_id UUID
)
RETURNS void
...
-- Create trigger function for auto-completion
...
SELECT cron.schedule(
  ...
  '0 * * * *',
  $$
  SELECT public.auto_complete_arrived_appointments();
  $$
);